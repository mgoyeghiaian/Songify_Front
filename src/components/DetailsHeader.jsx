  ...
  return (
    <div className="relative w-full flex flex-col">
      ...
      <div className="absolute inset-0 flex">
        <img
          alt="art"
          src={artistId ? artistt.artwork?.url.replace('{w}', '400').replace('{h}', '400') : songData?.images?.coverart || NoImage}
          className="sm:w-48 w-48 sm:h-48 h-28 rounded-full object-cover border-2 shadow-xl shadow-gray-600 p-2 bg-gray-500"
        />
        <div className="ml-8">
          <p className="font-bolder sm:text-3xl text-xl text-white mt-8">{artistId ? artistt.name : songData?.title}</p>
          {!artistId && (
